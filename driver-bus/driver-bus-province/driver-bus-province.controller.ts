...
@Controller('driver/bus-province')
export class DriverBusProvinceController {
  constructor(private readonly DriverBusProvinceService: DriverBusProvinceService) {}
  ...
  findAll(@TenantScope() tenantScope: TenantScopeResult) {
    ...
    return this.DriverBusProvinceService.findAll(tenantIds);
  }
  ...
  findOne(@Param('id', ParseObjectIdPipe) id: Types.ObjectId, @TenantScope() tenantScope: TenantScopeResult) {
    const { tenantIds } = tenantScope;
    return this.DriverBusProvinceService.findOne(id, tenantIds);
  }
  ...
  search(@Body(ParseObjectIdPipe) query: DriverSearchBusProvincesQuery, @TenantScope() tenantScope: TenantScopeResult) {
    const { tenantIds } = tenantScope;
    const {
      pageIdx = 0,
      pageSize = 0,
    ...
    return this.DriverBusProvinceService.search(+pageIdx, +pageSize, keyword, sortBy, filters, tenantIds);
  }
}
