...
@Controller('driver/bus-service')
export class DriverBusServiceController {
  constructor(private readonly DriverBusServiceService: DriverBusServiceService) {}
  ...
  findAll(@TenantScope() tenantScope: TenantScopeResult) {
    ...
    return this.DriverBusServiceService.findAll(tenantIds);
  }
  ...
  findOne(@Param('id', ParseObjectIdPipe) id: Types.ObjectId, @TenantScope() tenantScope: TenantScopeResult) {
    const { tenantIds } = tenantScope;
    return this.DriverBusServiceService.findOne(id, tenantIds);
  }
  ...
  search(@Body(ParseObjectIdPipe) query: DriverSearchBusServicesQuery, @TenantScope() tenantScope: TenantScopeResult) {
    const { tenantIds } = tenantScope;
    const {
      pageIdx = 0,
      pageSize = 0,
    ...
    return this.DriverBusServiceService.search(+pageIdx, +pageSize, keyword, sortBy, filters, tenantIds);
  }
}
