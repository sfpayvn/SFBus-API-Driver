...
@Controller('driver/bus-routes')
export class DriverBusRouteController {
  constructor(private readonly DriverBusRouteService: DriverBusRouteService) {}
  ...
  findAll(@TenantScope() tenantScope: TenantScopeResult) {
    ...
    return this.DriverBusRouteService.findAll(tenantIds);
  }
  ...
  findOne(@Param('id', ParseObjectIdPipe) id: Types.ObjectId, @TenantScope() tenantScope: TenantScopeResult) {
    const { tenantIds } = tenantScope;
    return this.DriverBusRouteService.findOne(id, tenantIds);
  }
  ...
  search(@Body(ParseObjectIdPipe) query: DriverSearchBusRouteQuery, @TenantScope() tenantScope: TenantScopeResult) {
    const { tenantIds } = tenantScope;
    const {
      pageIdx = 0,
      pageSize = 0,
    ...
    return this.DriverBusRouteService.search(+pageIdx, +pageSize, keyword, sortBy, filters, tenantIds);
  }
}
